...
pub fn borrow(
    ...
    debt_position.borrowed_amount = debt_position.borrowed_amount.checked_add(amount).ok_or(BorrowError::Overflow)?;
    debt_position.interest_accrued = debt_position.interest_accrued.checked_add(accrued_interest).ok_or(BorrowError::Overflow)?;
    ...
    collateral_position.amount = collateral_position.amount.checked_add(collateral_amount).ok_or(BorrowError::Overflow)?;
...
fn get_min_borrow_amount(env: &Env) -> i128 {
    env.storage().instance().get(&BorrowDataKey::BorrowMinAmount).unwrap_or(1000)
}

fn get_debt_ceiling(env: &Env) -> i128 {
    env.storage().instance().get(&BorrowDataKey::BorrowDebtCeiling).unwrap_or(i128::MAX)
}

fn get_total_debt(env: &Env) -> i128 {
    env.storage().instance().get(&BorrowDataKey::BorrowTotalDebt).unwrap_or(0)
}

fn set_total_debt(env: &Env, amount: i128) {
    env.storage().instance().set(&BorrowDataKey::BorrowTotalDebt, &amount);
}

pub fn set_admin(env: &Env, admin: &Address) {
    env.storage().instance().set(&BorrowDataKey::ProtocolAdmin, admin);
}
...
pub fn set_oracle(env: &Env, admin: &Address, oracle: Address) -> Result<(), BorrowError> {
    ...
    if *admin != current { return Err(BorrowError::Unauthorized); }
    admin.require_auth();
    env.storage().instance().set(&BorrowDataKey::OracleAddress, &oracle);
    Ok(())
}

pub fn get_liquidation_threshold_bps(env: &Env) -> i128 {
    env.storage().instance().get(&BorrowDataKey::LiquidationThresholdBps).unwrap_or(8000)
}

pub fn set_liquidation_threshold_bps(env: &Env, admin: &Address, bps: i128) -> Result<(), BorrowError> {
    let current = get_admin(env).ok_or(BorrowError::Unauthorized)?;
    if *admin != current { return Err(BorrowError::Unauthorized); }
    admin.require_auth();
    if bps <= 0 || bps > 10000 { return Err(BorrowError::InvalidAmount); }
    env.storage().instance().set(&BorrowDataKey::LiquidationThresholdBps, &bps);
    Ok(())
}
...
fn get_debt_position(env: &Env, user: &Address) -> DebtPosition {
    env.storage().persistent().get(&BorrowDataKey::BorrowUserDebt(user.clone()))
...
fn save_debt_position(env: &Env, user: &Address, position: &DebtPosition) {
    env.storage().persistent().set(&BorrowDataKey::BorrowUserDebt(user.clone()), position);
}

fn get_collateral_position(env: &Env, user: &Address) -> BorrowCollateral {
    env.storage().persistent().get(&BorrowDataKey::BorrowUserCollateral(user.clone()))
        .unwrap_or(BorrowCollateral { amount: 0, asset: user.clone() })
}

fn save_collateral_position(env: &Env, user: &Address, position: &BorrowCollateral) {
    env.storage().persistent().set(&BorrowDataKey::BorrowUserCollateral(user.clone()), position);
}
...
pub(crate) fn calculate_interest(env: &Env, position: &DebtPosition) -> i128 {
    ...
    let time_elapsed = env.ledger().timestamp().saturating_sub(position.last_update);
    ...
    let interest_256 = borrowed_256.mul(&rate_256).mul(&time_256)
...
pub fn initialize_borrow_settings(env: &Env, debt_ceiling: i128, min_borrow_amount: i128) -> Result<(), BorrowError> {
    env.storage().instance().set(&BorrowDataKey::BorrowDebtCeiling, &debt_ceiling);
    env.storage().instance().set(&BorrowDataKey::BorrowMinAmount, &min_borrow_amount);
    Ok(())
}

fn emit_borrow_event(env: &Env, user: Address, asset: Address, amount: i128, collateral: i128) {
    BorrowEvent { user, asset, amount, collateral, timestamp: env.ledger().timestamp() }.publish(env);
}

pub(crate) fn validate_collateral_ratio(collateral: i128, borrow: i128) -> Result<(), BorrowError> {
    let min_collateral = borrow.checked_mul(COLLATERAL_RATIO_MIN).ok_or(BorrowError::Overflow)?
        .checked_div(10000).ok_or(BorrowError::InvalidAmount)?;
    if collateral < min_collateral { return Err(BorrowError::InsufficientCollateral); }
    Ok(())
}
...
pub fn deposit(env: &Env, user: Address, asset: Address, amount: i128) -> Result<(), BorrowError> {
    if amount <= 0 { return Err(BorrowError::InvalidAmount); }
    let mut collateral_position = get_collateral_position(env, &user);
    ...
    collateral_position.amount = collateral_position.amount.checked_add(amount).ok_or(BorrowError::Overflow)?;
    save_collateral_position(env, &user, &collateral_position);
    crate::deposit::DepositEvent { user, asset, amount, timestamp: env.ledger().timestamp() }.publish(env);
    Ok(())
}
...
pub fn repay(env: &Env, user: Address, asset: Address, amount: i128) -> Result<(), BorrowError> {
    if amount <= 0 { return Err(BorrowError::InvalidAmount); }
    let mut debt_position = get_debt_position(env, &user);
    if debt_position.borrowed_amount == 0 && debt_position.interest_accrued == 0 { return Err(BorrowError::InvalidAmount); }
    if debt_position.asset != asset { return Err(BorrowError::AssetNotSupported); }
    let accrued_interest = calculate_interest(env, &debt_position);
    debt_position.interest_accrued = debt_position.interest_accrued.checked_add(accrued_interest).ok_or(BorrowError::Overflow)?;
    debt_position.last_update = env.ledger().timestamp();
    ...
    if remaining_repayment > 0 {
        if remaining_repayment > debt_position.borrowed_amount { return Err(BorrowError::RepayAmountTooHigh); }
    ...
    save_debt_position(env, &user, &debt_position);
    RepayEvent { user, asset, amount, timestamp: env.ledger().timestamp() }.publish(env);
    Ok(())
}
